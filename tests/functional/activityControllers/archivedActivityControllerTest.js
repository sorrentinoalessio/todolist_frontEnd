...
describe('Archived activity controller tests', () => {afterEach(async () => {
        sandbox.restore();
    ...
    describe('PATCH archived activity failure', () => {
        beforeEach(async () => {
    await fixturesUtils.clearDb();
});
        ...
        it('Should return 401 if token is not provided', async() => {
            ...
            const res = await request.execute(app).patch(`/${activityId}/archived`).send();
            expect(res.status).eq(401);
        })
        it('Should return 401 if token is not valid', async() => {
            const activityId = new objectId();
            const token = 'fake token';
            const res = await request.execute(app).patch(`/${activityId}/archived`).set('Authorization', `Bearer ${token}`).send();
            expect(res.status).eq(401);
        })
        
        it('Should return 404 if activity is open', async() => {
            ...
            const res = await request.execute(app)
                .patch(`/${activity._id.toString()}/archived`)
                .set('Authorization', `Bearer ${token}`)
                .send();
        ...
        it('Should return 404 if activity does not exists in db', async() => {
            ...
            const res = await request.execute(app)
                .patch(`/${new objectId().toString()}/archived`)
                .set('Authorization', `Bearer ${token}`)
                .send();
    ...
    describe('PATCH archived activity success', () => {
        ...
        it('Should return 200 if activity is already archived', async() => {
